...
\newcommand{\commitid}{\texttt{5dd5503}}
...
TTI frequency and scan P0 using the Timing relation \code{[P0, Total-P0]}. The generator
...
The mode supports SYNC (the Master controls the TTI) but is not available in the scheduled
queue.
...
\ui{Control TTI OUTPUT during scan}; the controller forces OUTPUT OFF, acquires 50 shots
...
\begin{itemize}
  ...
  \item \ui{Set Test Phase} immediately sets a phase from \ang{-360} to \ang{360}.
\end{itemize}
...
\end{equation}
This ordering is deliberately preserved in the archive metadata. Interleaving the two
phases at every frequency can reduce sensitivity to slow drift, but that ordering is an
experimental recommendation and is not implemented by this revision.

The controller opens one raw-LAN socket, normally on TCP port 9221, and retains it for the
complete scan. At the start of a phase sweep it first returns to $f_1$, then sends the
phase command. For each subsequent frequency it sends only the new frequency. The command
forms are \code{CHN n;FREQ value} and \code{CHN n;PHASE value}. TG5012A appends
\code{;*OPC?} and requires a reply of \code{1}; TGF3162 treats a completed socket write as
acceptance because its firmware may not answer \code{*OPC?} reliably. After every frequency
change the controller waits five seconds before acquiring the first repeat. Any connection,
identity, command or confirmation failure aborts the run. The socket is closed in all exit
paths, but carrier frequency and phase are not restored. Waveform, amplitude and output
state are never changed by this workflow.
...
\begin{tikzpicture}[node distance=7mm and 7mm]
  ...
  \node[block,left=of shots] (nextf) {Next frequency\\or next phase};
  \node[store,left=of nextf] (close) {Summarize\\close socket};
  \draw[flow] (connect)--(phase); \draw[flow] (phase)--(settle);
  \draw[flow] (settle)--(shots); \draw[flow] (shots)--(nextf);
...
only node that connects to or changes the TTI generator. Slaves start through the normal
