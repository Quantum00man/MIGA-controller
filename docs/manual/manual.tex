...
channel 1 of an Aim-TTi TG5012A. Configure the instrument address under
\ui{Settings $>$ TG5012A}, select Transfer Function under \ui{Scan Logic}, and enter the
...
The TG5012A settings tab also provides a test-frequency action: it connects with the
currently displayed address, verifies the model and immediately sets only the channel-1
...
new frequency it selects channel 1, sends only the \code{FREQ} command, waits for the instrument's operation-
complete reply, and then waits a fixed five seconds before the first sequence. It does not
...
TG5012A & Direct instrument IP, TCP port, timeout and connection/model test for Transfer
Function scans.\\
...
repository update status/fetch/apply, plus TG5012A connection test.\\
...
\pathcode{app/drivers/tti_generator.py} & TG5012A identity check and channel-1 carrier-frequency command.\\
