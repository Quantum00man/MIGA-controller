...
The phase-noise floor is derived independently from the sample standard deviation at each
frequency and generator phase. It describes normalized single-shot phase-noise variance,
not the standard error of the mean, and therefore contains no division by the valid sample
count:
\begin{equation}
  S_{\mathrm{noise}}^2(f)=
  \frac{s_{\Phi,0}^2(f)+s_{\Phi,90}^2(f)}{\phi_\omega^2}.
  \label{eq:transfer-noise-floor}
\end{equation}
For a single-phase acquisition, only the available component contributes. The original
transfer-function result is retained without noise-floor subtraction.
...
\end{equation}
and the phase tab additionally displays $S^2$. Its three curves are the \ang{0} component,
the \ang{90} component and their quadrature sum. A charcoal dashed curve shows the
sample-standard-deviation phase-noise floor from Equation~\eqref{eq:transfer-noise-floor}.
...
TG5012A/TGF3162 and CH1/CH2 selection, direct instrument IP, TCP port, timeout, connection
test, and immediate frequency/phase tests.\\
...
\ang{0}/\ang{90} components and quadrature $S^2$; optical-FM normalization.\\
