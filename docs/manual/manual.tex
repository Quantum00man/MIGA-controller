...
does not allow the final selected phase to be cleared. Transfer Function is an ordered,
one-dimensional Live scan. Randomization and alternate parameter sources are disabled,
and the uploaded \code{.mot} sequence is executed unchanged, so it requires no
...
second-to-last, and so on until reaching the center. For example,
\( [f_1,f_2,f_3,f_4,f_5] \) is acquired as
\( [f_1,f_5,f_2,f_4,f_3] \). With frequency-interleaved phases, each visited frequency
still completes its \ang{0} and \ang{90} measurements before moving on, and periodic
zero-phase calibration counts these completed frequency points in the selected order.
