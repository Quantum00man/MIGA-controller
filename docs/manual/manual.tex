...
settling delay. For each Slave, every matched shot is first differenced and normalized
by the common frequency-modulation wave-vector amplitude and the difference in the two
atom--mirror distances. With $\delta k=2\pi\Delta\nu_{780}/c$ and
$\Delta L=L_{\mathrm{Master}}-L_{\mathrm{Slave}}$, the signed quadrature is
\begin{equation}
 \Delta S_\psi(f)=\operatorname{mean}_{q}\left[
 \frac{\Phi_{\mathrm{Master},\psi,q}-\Phi_{\mathrm{Slave},\psi,q}}
 {2\delta k\Delta L}\right],\qquad \psi\in\{0^\circ,90^\circ\}.
\end{equation}
