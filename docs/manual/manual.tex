...
The Archive Re-Analysis panel can optimize one parameter in $[0,1]$ so that a selected
sequence mean or sample standard deviation reaches a requested target. The available parameters are the atom-number
...
deviation uses the same $N-1$ sample definition as Sequence Statistics. The calculation
uses only valid shots inside the currently applied Allan $P_0$ range and excludes the
...
interval, the closest feasible result and its residual are reported.
