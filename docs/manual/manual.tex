...
\newcommand{\commitid}{\texttt{7347b12}}
...
\begin{titlepage}
  ...
  \centering
  \vspace*{23mm}
  {\sffamily\bfseries\fontsize{35}{41}\selectfont MIGA Controller\par}
  \vspace{5mm}
  {\sffamily\fontsize{20}{25}\selectfont Operation, Optimization and Analysis Manual\par}
  \vspace{14mm}
  {\color{MigaCyan}\rule{0.76\textwidth}{1.3pt}\par}
  \vspace{17mm}
  {\Large Complete feature reference for the \branchname{} branch\par}
  \vfill
  \begin{tabular}{rl}
    \textbf{Author:} & Yiming MENG\\[2mm]
    \textbf{Rendered:} & \today\\[2mm]
    \textbf{Source revision:} & \commitid\\[2mm]
    \textbf{Project:} & MIGA / Cold Atoms Bordeaux
  \end{tabular}
  \vfill
  {\small Laboratory control software for cold-atom acquisition, sequence generation,\par
  online optimization, synchronized measurements and reproducible re-analysis.}
  \vspace*{12mm}
\end{titlepage}
\nopagecolor
\color{black}
...
\chapter{Bragg, AC Stark and lock-in workflows}\label{ch:special}
...
threshold map to \SI{-3}{V}; operating voltages must remain within \SIrange{-3}{3}{V}.
...
repository update status/fetch/apply.\\
...
phase space, reusable calibrations and sync retry.\\
...
\begin{itemize}
  ...
  \item Bragg voltage calibration operates within \SIrange{-3}{3}{V}; off is
