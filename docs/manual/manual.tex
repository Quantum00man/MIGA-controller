...
The Archive Re-Analysis panel can optimize one parameter in $[0,1]$. A selected sequence
...
selected, with Mean, Fit \ui{Interferometer P UP} and target zero as defaults. Standard
deviation uses the same $N-1$ sample definition as Sequence Statistics and rejects candidates
...
persists only the selected coefficient. If the target is unreachable inside the allowed
