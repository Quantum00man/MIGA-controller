...
class ScanConfig(BaseModel):
    ...
    intf_alpha_calibration_enabled: bool = Field(False)
    intf_alpha_calibration_interval_minutes: float = Field(2.0, gt=0.0, le=1440.0)
    intf_alpha_calibration_shots: int = Field(10, ge=2, le=1000)
...
class SystemSettings(BaseModel):
    ...
    intf_alpha_calibration_sequence_name: str = Field("")
    intf_alpha_calibration_sequence_content_base64: str = Field("")
    intf_alpha_calibration_fit_center_up: float = Field(0.0)
    intf_alpha_calibration_fit_width_up: float = Field(0.0, ge=0.0)
    intf_alpha_calibration_fit_center_dw: float = Field(0.0)
    intf_alpha_calibration_fit_width_dw: float = Field(0.0, ge=0.0)
    atom_area_method: str = Field("legacy")
    atom_area_baseline_points: int = Field(2)
    fit_model_key: str = Field("gaussian")
    fit_models: List[FitModelDefinition] = Field(default_factory=list)
