...
class PhaseNoiseAnalyzeTests(unittest.TestCase):
    ...
    def test_pages_expose_new_settings_mode_and_archive_plot(self):
        ...
        self.assertIn('@click="exportPhaseNoiseAllanCSV"', archive)
        self.assertIn("async exportPhaseNoiseAllanCSV()", archive)
        self.assertIn("'n_order', 'averaging_time_s', 'measured_allan_rad'", archive)
        self.assertIn("'power_meter_count', 'power_meter_mean_W', 'power_meter_std_W'", archive)
        ...
        self.assertIn("syncPhaseNodeRows(nodeId)", archive)
        self.assertIn("const pairingShot = item => this.isPhaseNoiseArchive() ? item.optimizationShot : item.shot;", archive)
        self.assertIn("referenceByShot.get(pairingShot(target))", archive)
        self.assertIn("Number(target.optimizationShot)", archive)
        self.assertIn("if (!record || record[field] == null", archive)
        self.assertIn("async exportPhaseNoiseCSV()", archive)
        self.assertIn("phase_noise_shots_", archive)
        self.assertIn("power_meter_mean_W", archive)
