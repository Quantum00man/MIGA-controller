...
Transfer Function and Bragg Fringes Calibration can also run a user-supplied periodic interferometer-labeling MOT. Each block derives $I_\alpha$ from fitted `Prob_UP_F2`, archives its uncertainty and validity, and updates the live monitor. Normal completion adds an endpoint block; final analysis uses piecewise-linear time interpolation of accepted calibration points, independently on Master and every Slave.
